...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  ...
  'Sports & Wellness': ['sport','fitness','gym','supplement','vitamin','protein','health','medicine','bandage','exercise'],
...
function smartLocalSearch(query: string): Product[] {
  ...
  return scored.filter(s => s.score > 0).sort((a, b) => b.score - a.score).slice(0, 24).map(s => s.product);
}
...
async function groqSearch(query: string): Promise<Product[]> {
  ...
  try {
    const catalog = (productsList as Product[]).slice(0, 200)
      .map(p => `${p.id}|${p.name}|${p.category}|${p.price}RWF`).join('\n');
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: `You are a shopping assistant for Simba Supermarket in Kigali, Rwanda. Find products matching the user's request. Understand natural language like "breakfast items", "something to drink", "baby care". Return ONLY valid JSON: { "productIds": [id1, id2, ...] }. Max 24 IDs. Only use IDs from the catalog.\n\nCATALOG:\n${catalog}` },
    ...
    if (jsonMatch) {
      ...
      const found = (parsed.productIds || [])
        .map((id: any) => (productsList as Product[]).find(p => String(p.id) === String(id)))
        .filter(Boolean) as Product[];
      if (found.length > 0) return found;
    }
  } catch (err) { console.error('Groq search error:', err); }
...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    groqSearch(queryParam).then(results => {
      ...
      setAiMessage(results.length > 0
        ? `✨ ${results.length} products found for "${queryParam}"`
        : 'No products found');
      setAiLoading(false);
    });
  }, [queryParam]);
  ...
  const regularProducts = useMemo(() => {
    ...
    if (activeCategory && activeCategory !== 'All') filtered = filtered.filter(p => p.category === activeCategory);
    ...
    <div className="container mx-auto px-4 py-6 text-foreground">
        ...
        <div>
          ...
          {aiLoading ? (
            <p className="text-sm text-[#F47A3E] flex items-center gap-1">
              <Loader2 className="w-3 h-3 animate-spin" /> Searching with AI...
            </p>
          ) : (
      ...
      {isAIMode && (
        <div className="mb-4 flex items-center gap-3 bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-800 rounded-xl px-4 py-3">
          ...
          <p className="text-sm text-gray-700 dark:text-gray-300 flex-1">
            <span className="font-bold text-[#F47A3E]">AI Search: </span>"{queryParam}"
          </p>
          <button onClick={clearSearch} className="flex items-center gap-1 text-xs font-bold text-gray-500 hover:text-red-500 transition-colors">
      ...
      {!isAIMode && (
        <div className="flex gap-2 flex-wrap mb-6">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => handleCategoryChange(cat)} className={`pill ${activeCategory === cat ? 'active' : ''}`}>
              {cat === 'All' ? t('allCategories') : getLocalizedCategoryName(cat)}
            </button>
          ))}
          {activeCategory !== 'All' && (
            <button onClick={() => handleCategoryChange('All')} className="flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold text-red-500 border border-red-200 hover:bg-red-50 transition-colors">
      ...
      {aiLoading ? (
        ...
        <div className="bg-white dark:bg-card border dark:border-border rounded-2xl p-16 text-center shadow-sm">
          ...
          <button onClick={clearSearch} className="bg-[#F47A3E] text-white px-6 py-2 rounded-xl font-bold hover:bg-[#D46A2E] transition">
            {t('clearFilters')}
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
          {displayProducts.map(product => <ProductCard key={product.id} product={product as any} />)}
        </div>
      )}
    </div>
  );
