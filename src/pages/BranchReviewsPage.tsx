...
export default function BranchReviewsPage() {
  ...
  const getRatingLabel = () => {
    if (rating === 1) return t('reviewPoor');
    if (rating === 2) return t('reviewFair');
    if (rating === 3) return t('reviewGood');
    if (rating === 4) return t('reviewVeryGood');
    return t('reviewExcellent');
  };

  return (
    <div className="container mx-auto px-4 py-10 max-w-4xl">
      <div className="mb-8">
        <h1 className="text-3xl font-black text-gray-900 dark:text-white mb-1">{t('branchReviewsTitle')}</h1>
        <p className="text-gray-500 dark:text-gray-400">{t('branchReviewsSubtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
        {branches.map(b => (
          ...
          >
            ...
            <p
              className="text-xs text-gray-400 mt-0.5 flex items-center gap-1 cursor-pointer hover:text-[#F47A3E] transition-colors"
              onClick={(e) => { e.stopPropagation(); openDirections(b); }}
            >
              <MapPin className="w-3 h-3" />{b.locationNote} - {t('getDirections')}
            </p>
          </div>
        ))}
      </div>
        ...
        <div className="card-premium p-6">
          {submitted ? (
            <div className="text-center py-8">
              ...
              <h3 className="font-bold text-lg dark:text-white mb-1">{t('reviewThankYou')}</h3>
              <p className="text-gray-500 text-sm mb-4">{t('reviewSubmitted')}</p>
              <button onClick={() => setSubmitted(false)} className="text-[#F47A3E] text-sm font-bold hover:underline">{t('writeAnotherReview')}</button>
            </div>
          ) : currentUser ? (
            <>
              <h3 className="font-bold text-lg dark:text-white mb-1">{t('rateBranch', { branch: branch?.name })}</h3>
              <p className="text-gray-500 text-sm mb-4">{t('howWasExperience')}</p>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <textarea
                  ...
                  placeholder={t('shareExperienceOptional')}
            ...
            <div className="text-center py-8">
              ...
              <h3 className="font-bold dark:text-white mb-1">{t('signInToReview')}</h3>
              <p className="text-gray-500 text-sm mb-4">{t('shareExperienceWithOtherShoppers')}</p>
